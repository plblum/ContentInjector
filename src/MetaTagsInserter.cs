...
/// <remarks>
/// </remarks>
   public class MetaTagsInserter : BaseKeyedInserter<MetaTagInserterItem>, IMetaTagsInserter
   {
#if false
      ...
      public override string GetContent(HttpContextBase httpContext)
      {
         ...
         {
            foreach (MetaTagInserterItem items in orders.Value)
            {
      ...
      protected override void ItemContent(MetaTagInserterItem item, StringBuilder sb, HttpContextBase httpContext)
      {
         string usage = "name";
         switch (item.Usage)
         {
...
/// <param name="order"></param>
      ...
      {
         if (!Contains(name))
            Add(new MetaTagInserterItem(name, content), order);
...
/// <param name="order"></param>
      ...
      {
         if (!Contains(name))
            Add(new MetaTagInserterItem(usage, name, content), order);
      }
